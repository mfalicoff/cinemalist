...
public class ScraperService(IEnumerable<Scraper.Scrapers.Scraper> scrapers, IIMBDService imbdService, ILogger<ScraperService> logger): IScraperService
{
    ...
    private async Task PersistFilmsAsync(List<Film> films, CancellationToken cancellationToken)
    {
        foreach (Scraper.Scrapers.Scraper scraper in _scrapers)
        {
            await scraper.PersistRunAsync(films, cancellationToken);
        }
    }
}